...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for input_values, labels in tqdm(test_loader, desc="Testing"):
            ...
            tp, tn, fp, fn = calculate_metrics(outputs, labels, 0.5)  # Using 0.5 as initial threshold
            tp_total += tp
            tn_total += tn
            fp_total += fp
            fn_total += fn
    ...
    plt.title("ROC Curve")
    plt.legend()
    plt.savefig("ours_without_downsampling.png")
    plt.close()
...
# Load model with module support
...
# Dataset class for audio files with two-level directory structure
...
# Create data loaders for labeled datasets
