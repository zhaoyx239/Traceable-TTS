...
from train import collate_fn, AudioDataset  # 使用你的训练代码中的相关部分
...
def test_model(model, test_loader, device):
    ...
    plt.title("ROC Curve")
    plt.legend()
    plt.savefig("ours_without_downsampling.png")  # 保存为图片
    ...
    results_df.to_csv('prediction_results_baseline.csv', index=False)
    print("Prediction results saved to 'prediction_results_baseline.csv'")'''
...
def create_data_loaders(data_paths_label_1, data_paths_label_0, processor, batch_size=8):
    ...
    # 合并数据集
    combined_dataset = torch.utils.data.ConcatDataset(datasets_label_1 + datasets_label_0)
...
def main():
    ...
    # 加载模型
    model = load_model_with_module(model, '/hpc_stor03/sjtu_home/yuxiang.zhao/classification_project/ckpts/it3/model_best.pth', device)
    model.to(device)
    ...
    change = "0.8"
    data_paths_label_1 = [
        f"/hpc_stor03/sjtu_home/yuxiang.zhao/data/MM/f5tts/it3/test-clean"
    ]
    data_paths_label_0 = [
        f"/hpc_stor03/sjtu_home/yuxiang.zhao/data/groundtruth/LibriTTS/test-clean",
        f"/hpc_stor03/sjtu_home/yuxiang.zhao/data/infer/cosyvoice/test-clean",
        f"/hpc_stor03/sjtu_home/yuxiang.zhao/data/infer/e2tts/test-clean",
        f"/hpc_stor03/sjtu_home/yuxiang.zhao/data/infer/cosyvoice2/test-clean"
    ]

    # 创建测试数据加载器
    test_loader = create_data_loaders(data_paths_label_1, data_paths_label_0, processor, batch_size=128)
